...
### ▶️ 소수점 관련 문제 
- 0. 00. 을 입력하고 연산자를 눌렀을때 화면이 업데이트 되지 않도록 구현
- 000.2 와 같이 불필요한 0 제거되도록 구현

#### ⚒️ 해결방법
- `hasSuffix` 를 이용하여 .으로 끝나는 조건을 걸어 해결하였습니다.
- `addStackView`에서 `Double` 타입으로 한번 변환해서 000.2 -> 0.2가 되도록 잡아주는 조건을 걸어 해결하였습니다. 이렇게 하다보니 문제는 5를 입력하면 스크롤뷰에 5.0 으로 입력이 되는 문제가 생겼으나 결과값에서 `numberFormatter`를 사용하는 것으로..만족하였습니다.

### ▶️ 조건에 따라 실행되는 구문이 많을때, 연산프로퍼티 활용
    
연산 프로퍼티 활용을 추천드렸던 부분은 이전 코드에서 아래와 같이 특정 상황에서 Bool 값을 갖는다면,
```swift
...
따로 false, true 값을 넣어주는 대신 연산 프로퍼티로 지정

```swift
var isEnteredOperand: Bool {
    return numberInput.text != Expression.zero 
}
```
- 이런식으로 `Bool` 타입 변수에 `return` 값으로 넣어주어 연산프로퍼티를 적용하는 방법도 있다.
